...
// public:
	...
	public function __construct( $id, $connection, &$language ) {
		...
		// Load rating
		$this->rating_ = new Vote( $id, $connection, true );
	}
	...
	// Returns star count
	public function getRating( $real_average = false ) {
	   if( $real_average ) {
	      return $this->rating_->getAverage();
	   }
	   return $this->rating_->getEqualizedInt();
	}
	
	public function getVotes() {
		return $this->rating_->getVotes();
	}
	
	public function rate( $points ) {	
		return $this->rating_->vote( $points );
	}
